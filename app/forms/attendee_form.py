...
class AttendeeRegistrationForm(FlaskForm):
    """Registration form for event attendees."""
    ...
    category = HiddenField(default=RegistrationType.ATTENDEE.value)

    def validate_email(self, field):
        """Prevent duplicate attendee registrations."""
        existing = (
            db.session.query(Registration)
            .filter_by(email=field.data.lower(), category=RegistrationType.ATTENDEE)
            .first()
        )
        if existing:
