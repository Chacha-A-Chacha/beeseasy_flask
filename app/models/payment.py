...
class Payment(db.Model):
    ...
    # Metadata
    payment_metadata = db.Column(JSON)  # Additional payment metadata
    gateway_response = db.Column(JSON)  # Raw gateway response
    ...
    @validates(
        "subtotal", "total_amount", "tax_amount", "discount_amount", "refund_amount"
    )
    ...
    def to_dict(self, include_sensitive: bool = False) -> Dict[str, Any]:
        ...
        data = {
            ...
            "payment_completed_at": self.payment_completed_at.isoformat()
    ...
    def __repr__(self):
        ...
        ref = (
            self.payment_reference[:10] + "..."
            if len(self.payment_reference) > 10
...
class PromoCode(db.Model):
    ...
    applicable_ticket_types = db.Column(JSON)  # Specific ticket types
    applicable_packages = db.Column(JSON)  # Specific exhibitor packages
    ...
    def validate_code(self, key, value):
        ...
        if value:
            ...
            if not value.replace("-", "").replace("_", "").isalnum():
                raise ValueError(
                    "Promo code must be alphanumeric (dashes and underscores allowed)"
                )
        return value
    ...
    def is_valid_for_user(self, email: str) -> bool:
        ...
        if self.max_uses_per_user:
            ...
            usage_count = (
                db.session.query(PromoCodeUsage)
                .join(Registration)
                .filter(
    ...
    def use_code(self) -> bool:
        ...
        result = db.session.execute(
            ...
            .where(
                ...
                PromoCode.is_active == True,  # type: ignore[arg-type]
                or_(
...
class PromoCodeUsage(db.Model):
    ...
    promo_code_id = db.Column(
        db.Integer, db.ForeignKey("promo_codes.id"), nullable=False, index=True
    )
    registration_id = db.Column(
        db.Integer, db.ForeignKey("registrations.id"), nullable=False, index=True
    )
...
class ExchangeRate(db.Model):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_currency_pair_date", "from_currency", "to_currency", "effective_date"
        ),
    )
    ...
    def get_current_rate(
        ...
        rate = (
            cls.query.filter_by(from_currency=from_currency, to_currency=to_currency)
