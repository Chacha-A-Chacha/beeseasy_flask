...
class ContactService:
    ...
    def send_contact_message(form_data: dict) -> tuple:
        ...
        try:
            ...
            reference_number = f"BEE{datetime.now().strftime('%Y%m%d')}{secrets.token_hex(3).upper()}"
            ...
            recipient = ContactService.ROUTING_MAP.get(
                inquiry_type, 
                ContactService.ROUTING_MAP['other']
            )
            ...
            email_context = {
                **form_data,
                ...
                'event_name': current_app.config.get('EVENT_NAME', 'Bee East Africa Symposium 2025')
            }
            ...
            user_msg.html = render_template('emails/contact_confirmation.html', **email_context)
            user_msg.body = ContactService._create_plain_text_confirmation(email_context)
